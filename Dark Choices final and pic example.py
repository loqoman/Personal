...
DIMGRAY = (105,105,105)
BACKGROUND = DIMGRAY
#Setting up vaible used later on in program that have to be up here
hammer_chance = random.randrange(8,12)
hammer = 0
...
pygame.display.set_caption('Dark Choices')
    ...
    def __init__(self,window=windowSurface,num_lines=4,
        ...
        for i in range (0,self.num_lines):
            if i == 0:
                ...
                topleft = (topleft[0],self.lines[i-1].bottom)
...
class Collection(object):
    ...
    # returned.
        for i in range(0,num):
            if item in self.collection: # ensure we have one
                self.collection[item] -= 1 # lower the count
                if self.collection[item] <= 0:
                    del self.collection[item]
...
def starting_search_handler(self):
    global hammer_chance
    global hammer
    hammer += 1
    chance = random.randrange(0,100)
    if chance <= 30:
         planks = random.randrange(1,4)
         scroll.writeln('You find some wooden planks laying around')
         for i in range(0,planks):
             inventory.add('plank')
    elif chance > 30 and chance < 60:
         nails = random.randrange(5,9)
         scroll.writeln('You find a box of nails in the wreakage')
         for i in range(0,nails):
             inventory.add('nail')
    elif chance > 60 and chance < 90:
         nails = random.randrange(1,3)
         planks = random.randrange(1,2)
         scroll.writeln('You find a few planks with serval nails in them.')
         scroll.writeln('With luck you could be able to get them out.')
         for i in range (0,nails):
             inventory.add('nail')
         for i in range (0,planks):
             inventory.add('plank')
    elif chance > 90 and chance < 95:
         snail = random.randrange(1,3)
         scroll.writeln('Thats odd, there are snails crawling about?')
         for i in range(0,snail):    
             inventory.add('snail')
    elif chance > 95:
         scroll.writeln('The handle of a tolet plunger sticks out, why in the world would you need a plunger?')
         inventory.add('Toilet_Plunger')
    if hammer == hammer_chance:
        scroll.escape()
        scroll.writeln('This time you find a hammer among the wreakage.')
        time.sleep(1)
        scroll.writeln('Mabey you could start to build a house?')
        inventory.add('Hammer')
        change_button('but2','Build',build_handler)
    self.toggle()
    inventory_scroll.escape()
    inventory_scroll.writeln('Inventory currently contains:')
    inventory.scroll_contents(inventory_scroll)
    #inventory.scroll_contents(scroll)
def build_handler(self):
    change_button('Option2','Lean-to',build_option_handler)
    change_button('Option1','Tree House',build_option_handler)
def build_option_handler(self): 
    nPlanks = inventory.number_of('plank')
    nNails = inventory.number_of('nail')
    building = self.label
    self.toggle()
    if building == ('Tree House') and nPlanks >= 10 and nNails >= 12 :
        inventory.remove('plank',10)
        inventory.remove('nail',12)
        scroll.writeln('You begn to build the first few steps to a larger platform on a tree')
    elif building == ('Tree House') or nPlanks < 10 or nNails < 12:
        scroll.writeln('You look at the feeble pile of supplies around you. It dosent look like enough.')
        scroll.writeln('Maby try searching again?')
    elif building == ('Lean-to') and nPlanks >= 8 and nNails >= 15 :
        inventory.remove('plank',8)
        inventory.remove('nail',15)
        scroll.writeln('You begin to prop up a few planks for a much larger shelter')
#-------------------------------End of handlers(whew you made it!)------------------------#
...
# main game initialization routine to set up screen and some game variables
    ...
    but1 = Button(name='but1',color=BLACK,
                  topleft = (500,25),
                  size = 12,label = 'Search',
                  app_handler = starting_search_handler,
                  group='Action')
...
scroll = Scroll(num_lines=8,topleft=(0,20),text = '',
        background_color=WHITE,color = BLACK,
        font_size=18,max_chars = 50,
        outline=False)
inventory_scroll = Scroll(num_lines=20,topleft=(590,400),text = '',
        background_color=WHITE,color = BLACK,
        font_size=18,max_chars = 40,
        outline=True)

inventory = Collection(name='Inventory')
...
time.sleep(3)
scroll.writeln('You wakes up under a large, spreading tree at the edge of a field.')
time.sleep(2)
